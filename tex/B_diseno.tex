...
La aplicación sigue el patrón arquitectónico \textbf{MVC (Modelo-Vista-Controlador)} implementado sobre el framework Flask en Python. \cite{grinberg:flask}
...
\begin{description}
	...
	\item- Información sobre ELA: Explica qué es la Esclerosis Lateral Amiotrófica, sus tipos según el origen, sus fases de progresión y consideraciones sobre la preservación cognitiva. \cite{elaandalucia:guia}
	\item- Información sobre SAAC: Describe los Sistemas Aumentativos y Alternativos de Comunicación, su clasificación según tecnología, la importancia de los sistemas escalables y aspectos sobre financiación. \cite{lesturnerals:elacomunicacion}
	\item- Base legal: Detalla la finalidad del tratamiento de datos, las bases jurídicas, la política de conservación, los destinatarios de la información y los derechos del usuario en cumplimiento del RGPD. \cite{ue:rgpd}
\end{description}
...
\begin{description}
	\item- Cuestionario: Formulario web que recoge el perfil clínico del usuario mediante menús desplegables y botones de opción, incluyendo la casilla de consentimiento RGPD obligatorio. El diseño de la interfaz y la disposición de sus elementos sigue las directrices de 				accesibilidad de software para garantizar la usabilidad del sistema \cite{iso:9241}.
	...
	\item- Agradecimiento: Pantalla de confirmación tras el envío del feedback.		\item- Panel de administración: Vista exclusiva para el investigador que lista cronológicamente todos los registros almacenados con consentimiento, diferenciando casos reales de pruebas.
\end{description}
...
\subsection{Lógica de asociación de hardware y recursividad}
Para garantizar que la recomendación sea completa, el sistema no solo filtra el software principal, sino que deduce de forma recursiva los periféricos y accesorios necesarios para su funcionamiento. Esto se realiza mediante un algoritmo de búsqueda en cascada.

La lógica reside en una función recursiva que opera sobre la tabla \texttt{sistema\_dependencia}. El proceso sigue los siguientes pasos:
\begin{enumerate}
    \item \textbf{Detección de dependencia:} Por cada sistema recomendado, el algoritmo busca en la base de datos si existe hardware obligatorio asociado (ej. \textit{Grid 3} requiere \textit{Eye Tracker}).
    \item \textbf{Filtros de viabilidad:} Antes de añadir un periférico, el sistema valida:
    \begin{itemize}
        \item \textbf{Filtro táctil:} Si el periférico requiere uso manual (ej. \textit{Lápiz táctil}), solo se incluye si el usuario declaró tener funcionalidad en las manos.
        \item \textbf{Filtro de plataforma:} Solo se incluyen accesorios compatibles con el sistema operativo detectado o seleccionado (ej. no sugerir un \textit{Apple Pencil} si el usuario usa \textit{Android}).
    \end{itemize}
    \item \textbf{Recursión (Salto de nivel):} Una vez añadido un hardware, la función se llama a sí misma para comprobar si ese hardware necesita otro a su vez (ej. el \textit{Eye Tracker} requiere un \textit{Soporte de anclaje articulado}).
    \item \textbf{Control de redundancia:} El sistema utiliza un conjunto de identificadores (\texttt{Set}) para evitar bucles infinitos y asegurar que cada accesorio aparezca una sola vez en la interfaz, independientemente de cuántos programas lo requieran.
\end{enumerate}

\imagen{../img/diagrama_recursividad.png}{Diagrama de flujo del algoritmo de asociación recursiva de periféricos.}

Se puede acceder a la aplicación web desarrollada a través del siguiente enlace \url{https://cuestionario-tfg.onrender.com/}