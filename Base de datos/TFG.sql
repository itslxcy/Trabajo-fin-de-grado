...
-- 4. CARGA DE SISTEMAS (Software, Apps, Hardware y Periféricos)
INSERT INTO saac_sistema 
(nombre, descripcion, requiere_interlocutor, fatiga_fisica, portable, admite_anclaje, enlace_info)
VALUES
-- BAJA TECNOLOGÍA
('Panel pictogramas','Tablero físico que utiliza símbolos pictográficos (ARASAAC).',true,3,true,true, 'https://arasaac.org/'),
('Panel alfabético','Tablero de comunicación directa mediante el deletreo.',true,3,true,true, 'https://downloads.tobiidynavox.com/Conditions/ALS/Communication_Board/TD_CommunicationBoard_ALS_es-ES.pdf'),
('SpeakBook','Cuaderno de comunicación diseñado para la selección mediante la mirada.',true,2,true,true, 'https://aulaabierta.arasaac.org/materiales-caa-tableros-de-comunicacion'),
('Tablero ETRAN','Panel transparente para comunicación mediante la dirección de la mirada.',true,1,true,true, 'https://downloads.tobiidynavox.com/Conditions/ALS/Communication_Board/TD_CommunicationBoard_ALS_es-ES.pdf'),
('QuickTalker 23','Comunicador portátil con 23 casillas para mensajes grabados.',false,3,true,false, 'https://qinera.com/es/tienda/comunicadores-sencillos/quick-talker-23'),
('MegaBEE', 'Escritura asistida por mirada y teclado táctil asistido por un interlocutor.', true, 2, true, true, 'https://www.e2l.uk.com/megabee/indexes.html'),

-- SOFTWARE Y APPS (Windows, iOS, Android)
...
('Predictable', 'Comunicación alfabética con predicción inteligente y acceso por pulsador.', false, 2, true, true, 'https://apps.apple.com/es/app/predictable/id404445007'),
...
('Conmutador por soplido (Pufo)', 'Sensor de soplido suave para activar comunicador.', false, 0, true, true, 'https://qinera.com/es/tienda/acceso-a-dispositivos-y-conmutadores/quha-pufo-con-kit-de-montaje-de-cabeza'),
('Conmutador de pedal', 'Pulsador para ser activado con el pie.', false, 2, true, false, 'https://qinera.com/es/tienda/ratones-y-pulsadores/conmutador-de-pedal'),
('Conmutador Spec Amarillo', 'Pulsador de fácil activación física.', false, 2, true, true, 'http://qinera.com/es/tienda/conmutadores/conmutador-spec-amarillo'),
...
('Guantes antiincrustantes', 'Guante que evita toques accidentales en pantallas táctiles.', false, 0, true, false,'https://www.amazon.es/HUION-Tableta-Antiincrustante-Personas-Tabletas/dp/B00VTHAS00'),
('Lápiz táctil', 'Ergonómico para mayor precisión en tablets.', false, 3, true, false, 'https://goo.su/jnGcNM'),
('Ratón Bluetooth', 'Ratón inalámbrico ergonómico o tipo trackball.', false, 2, true, true, 'https://goo.su/3XvwDat'),
...
WHERE nombre IN ('Grid 3', 'Verbo', 'TD Snap', 'ComuniQa (TD Snap)', 'ComuniQa (Grid 3)', 'Communicator 5', 'TD Pilot', 'Predictable', 'EVA Facial Mouse');

INSERT INTO sistema_requisito_funcional (sistema_id, nivel_visual_min, nivel_auditivo_min, nivel_cognitivo_min, nivel_tecnologico_min, nivel_habla_min)
SELECT id, 1, 1, 2, 1, 0 FROM saac_sistema 
WHERE nombre IN ('Boardmaker 7', 'QuickTalker 23', 'Conmutador por soplido (Pufo)', 'Conmutador de pedal', 'Conmutador Spec Amarillo', 'BJOY Chin Plus', 'Guantes antiincrustantes', 'Lápiz táctil', 'Ratón Bluetooth', 'VirtualTEC', 'Ease Touch');
...
WHERE nombre IN ('Panel pictogramas', 'Panel alfabético', 'SpeakBook', 'Puntero Láser', 'Asistente de voz AAC', 'Proloquo2Go', 'Grid 3', 'Verbo', 'Boardmaker 7', 'QuickTalker 23', 'LetMeTalk', 'TD Snap', 'Guantes antiincrustantes', 'Lápiz táctil', 'Ratón Bluetooth', 'Predictable', 'VirtualTEC', 'Ease Touch');

INSERT INTO sistema_entrada (sistema_id, entrada_id)
SELECT id, (SELECT id FROM tipo_entrada WHERE nombre = 'ojos') FROM saac_sistema 
WHERE nombre IN ('Look to Speak', 'Eye tracker', 'Tablero ETRAN', 'Tallk', 'Look to learn', 'Grid 3', 'ComuniQa (TD Snap)', 'ComuniQa (Grid 3)', 'TD Pilot', 'Communicator 5', 'MegaBEE', 'OptiKey');

INSERT INTO sistema_entrada (sistema_id, entrada_id)
SELECT id, (SELECT id FROM tipo_entrada WHERE nombre = 'pulsador') FROM saac_sistema 
WHERE nombre IN ('QuickTalker 23', 'Conmutador de pedal', 'Conmutador por soplido (Pufo)', 'BJOY Chin Plus', 'Grid 3', 'Communicator 5', 'Predictable', 'VirtualTEC');

INSERT INTO sistema_entrada (sistema_id, entrada_id)
SELECT id, (SELECT id FROM tipo_entrada WHERE nombre = 'cabeza') FROM saac_sistema 
WHERE nombre IN ('Gafas con Puntero Láser', 'BJOY Chin Plus', 'EVA Facial Mouse');
...
INSERT INTO sistema_idioma (sistema_id, idioma_id)
SELECT s.id, (SELECT id FROM idioma WHERE nombre = 'gallego') FROM saac_sistema s WHERE nombre = 'VirtualTEC';
...
WHERE nombre IN ('Grid 3', 'Eye tracker', 'Verbo', 'Look to learn', 'TD Snap', 'Communicator 5', 'Boardmaker 7', 'OptiKey');

INSERT INTO sistema_plataforma (sistema_id, plataforma_id)
SELECT id, (SELECT id FROM plataforma WHERE nombre = 'iOS') FROM saac_sistema 
WHERE nombre IN ('Proloquo2Go', 'TD Snap', 'TD Pilot', 'LetMeTalk', 'Guantes antiincrustantes', 'Lápiz táctil', 'Predictable');

INSERT INTO sistema_plataforma (sistema_id, plataforma_id)
SELECT id, (SELECT id FROM plataforma WHERE nombre = 'Android') FROM saac_sistema 
WHERE nombre IN ('Look to Speak', 'Tallk', 'LetMeTalk', 'Guantes antiincrustantes', 'Lápiz táctil', 'VirtualTEC', 'EVA Facial Mouse', 'Ease Touch');
